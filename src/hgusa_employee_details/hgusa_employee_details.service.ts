...
export class HgusaEmployeeDetailsService {
  ...
  async process(
    ...
    try {
      ...
      const headers = Object.keys(data[headerRowIndex] || {}).filter(
        (h) => h.trim() !== '',
      );
      ...
      if (mapped.length === 0) {
        this.logger.warn('No valid rows to insert or update.');
        return;
      }

      await this.upsertData(mapped);
      this.logger.log(
        `Finished processing HGUSA Employee Details: ${fileName}`,
      );
    } catch (error: any) {
      ...
      throw error instanceof BadRequestException
        ...
        : new InternalServerErrorException(
            'Failed to process HGUSA Employee Details',
          );
    }
  }

  private findHeaderRowIndex(data: any[]): number {
    const expectedNormalized = this.expectedHeaders.map((h) =>
      h.replace(/\s+/g, '').toLowerCase(),
    );
    for (let i = 0; i < data.length; i++) {
      const keys = Object.keys(data[i] || {}).map((k) =>
  ...
  private mapToEntity(
    ...
    const businessUnit = (raw['Business Unit Description'] ?? '')
      .toString()
      .trim();
    ...
    if (!businessUnit || !fullName) {
      this.logger.warn(
        `Skipping row due to missing primary key data: ${JSON.stringify(raw)}`,
      );
      return null;
    }
  ...
  private async upsertData(data: HgusaEmployeeDetail[]) {
  await this.entityManager.transaction(async (manager) => {
 
    const validRows = data.filter((row) => row.businessUnit && row.fullName);

    if (validRows.length === 0) {
      this.logger.log(`No valid rows to insert or update.`);
      return;
    }
    ...
    const uniqueRows = validRows.filter((row) => {
      ...
      if (seen.has(key)) {
        this.logger.warn(
          `Skipping duplicate row in file: ${JSON.stringify(row)}`,
        );
        return false;
      }
      seen.add(key);
    ...
    await manager
      .createQueryBuilder()
      .insert()
      .into(HgusaEmployeeDetail)
      .values(uniqueRows)
      .orIgnore()  
      .execute();

    this.logger.log(`Inserted or skipped ${uniqueRows.length} rows.`);
  });
}
